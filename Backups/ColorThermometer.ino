...
// write digits using shift-register
// 7-segment led A,B,C,D,E,F clockwise from top, G(mid-of-8), H(dot)
// 0 thru 9 + dot c f   digit-1 = B,C so 01100000=6
const byte digitBytes[] = { 63, 6, 91, 79, 102, 109, 124, 7, 127, 103, 128, 88, 113, 0 };
...
void setup() {
  ...
  //set pins for shift register for output
  ...
  for (int d=0; d<sizeof(digitBytes); d++) {
    writeDigit(digitBytes[d], 250);
  }
...
void loop() {
  ...
  */
  ...
  String tempStr = "C= " + String(tempC, 1); // no decimals
  tempStr += " F= " + String(tempF, 1) + " ";
  ...
  String fTempStr = String(tempF, 1); // 76.5f
  writeTempString(fTempStr, 500);
  String cTempStr = String(tempC, 1) + "c"; //24.7c
  //writeTempString(cTempStr, 500);

  writeDigit(0, 1); //blank
  delay(2000);
}

void writeDigit(byte digitByte, int idelay) {
  // take the latchPin low so the LEDs don't change while you're sending in bits
  digitalWrite(latchPin, LOW);
  // shift out the bits
  shiftOut(dataPin, clockPin, MSBFIRST, digitByte);
  //take the latch pin high so the LEDs will light up:
  digitalWrite(latchPin, HIGH);
  delay(idelay);
}

void writeTempString(String temp, int idelay) {
  for (int c=0; c<temp.length(); c++) {
    byte asciiByte = byte(temp[c]);
    Serial.println("char=" + String(temp[c]) + " ascii = " + String(asciiByte));
    // "0" is 48
    // . is -2
    // c is 
    ...
    switch(asciiByte){
      case 46:
        // dot
        writeDigit(digitBytes[10], idelay); //dot
        break;
      case 99:
        // c
        writeDigit(digitBytes[11], idelay); //c
        break;
      case 102:
       // f
        writeDigit(digitBytes[12], idelay);
        break;
      default:
        writeDigit(digitBytes[asciiByte - 48], idelay);
        break;        
    } //switch
  } //for
}
